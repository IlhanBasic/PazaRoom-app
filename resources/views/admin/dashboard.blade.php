    ...
    <div x-data="{
        ...
        <div :class="{ 'open': sidebarOpen }" class="sidebar">
            <div class="sidebar-content">
                ...
                <nav>
                    <a @click="setCurrentTab('acceptProperty')" :class="{ 'active': currentTab === 'acceptProperty' }" class="sidebar-link">
                        <i class="fa-solid fa-check"></i>
                        <span>Proveri nekretninu</span>
                    </a>
                    <a @click="setCurrentTab('users')" :class="{ 'active': currentTab === 'users' }" class="sidebar-link">
                        <i class="fas fa-users"></i>
                        <span>Korisnici</span>
                    </a>
                    <a @click="setCurrentTab('roles')" :class="{ 'active': currentTab === 'roles' }" class="sidebar-link">
                        <i class="fa-solid fa-list"></i>
                        <span>Vrste korisnika</span>
                    </a>
                    <a @click="setCurrentTab('properties')" :class="{ 'active': currentTab === 'properties' }" class="sidebar-link">
                        <i class="fas fa-building"></i>
                        <span>Nekretnine</span>
                    </a>
                    <a @click="setCurrentTab('propertyTags')" :class="{ 'active': currentTab === 'propertyTags' }" class="sidebar-link">
                        <i class="fas fa-tags"></i>
                        <span>Tagovi</span>
                    </a>
                    <a @click="setCurrentTab('reviews')" :class="{ 'active': currentTab === 'reviews' }" class="sidebar-link">
                        <i class="fas fa-tags"></i>
                        <span>Recenzije</span>
                    </a>
                            ...
                            <tbody>
                                @foreach ($properties->where('status', 'Pending') as $property)
                                    <tr>
                                        ...
                                        <td class="actions">
                                            <form action="{{ route('property_accept', $property->id) }}" method="POST" class="accept-property-form">
                                                @csrf
                                                @method('PATCH')
                                            ...
                                            <form action="{{ route('property_decline', $property->id) }}" method="POST" class="reject-property-form">
                                                @csrf
                                                @method('PATCH')
                                    ...
                                    <tr>
                                        ...
                                        <td class="actions">
                                            <form action="{{ route('delete_user', $user->id) }}" method="POST"
                                                  id="delete-form-{{ $user->id }}" class="delete-property-tag-form">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn-delete">
                                                    <i class="fas fa-trash"></i>
            ...
            <tbody>
                @foreach ($propertyTags as $propertyTag)
                    <tr>
                        ...
                        <td class="actions">
                            <a href="{{ route('property_tag_edit', $propertyTag->id) }}" class="btn-edit">
                                <i class="fas fa-edit"></i>
                            </a>
