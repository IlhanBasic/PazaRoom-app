<x-layout>
    <div class="container mx-auto py-8">
        <h1 class="text-2xl font-semibold mb-4 text-center">Izmeni ulogu</h1>
        <form action="{{ route('role_update', $role) }}" method="POST" enctype="multipart/form-data" id="form"
            class="bg-white shadow-md rounded-lg p-6 space-y-6 md:space-y-8 border border-gray-200 mx-auto max-w-4xl">
            @csrf
            @method('PATCH')
            {{-- Tag  --}}
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                <label for="name" class="text-sm font-medium text-gray-700">Naziv uloge</label>
